...
namespace IKARUSWEB.UI.Controllers
{
    public sealed class CurrenciesController : Controller
    {
        ...
        public CurrenciesController(IApiClient api, IViewLocalizer l) { _api = api; _l = l; }

        [HttpGet]
        public async Task<IActionResult> Index(string? q, CancellationToken ct)
        {
            var list = await _api.GetCurrenciesAsync(q, ct);
            return View(list.Select(d => new CurrencyViewModel
            {
                Id = d.Id,
                Name = d.Name,
                Code = d.Code,
                CurrencyMultiplier = d.CurrencyMultiplier,
                Rate = d.Rate
            }).ToList());
        }
